...
from ..data.loraks2 import MAGICDatasetLORAKS
...
def setup(parser):
    ...
    parser.add_argument("--batch-size", type=int, default=8, help="Number of Batchsize")
    parser.add_argument(
...
def main_worker(gpu, ngpus, args):
    ...
    model = JVSNet(num_echos=10, alfa=0.1, beta=0.1, cascades=args.cascades)
    ...
    train_dataset, val_dataset = torch.utils.data.random_split(
        train_dataset, [total - numval, numval], generator=torch.Generator().manual_seed(0)
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if is_printer:
            ...
            if epoch % 10 == 0:
                savemat(f"save_file/{epoch}.mat", {'test': testimage.numpy(), 'val': valimage.numpy()})
...
def show_output(model, testdata, gpu):
    ...
    return make_grid([_make_image(i) for i in range(10)], nrow=1, padding=0)